(window.FRAMEWORKS = window.FRAMEWORKS || []).push({
  ...
  sections: [
    ...
    },
    {
      ...
      level: "core",
      body: [
        { type: "p", text: "You write tests **in Solidity**. A test contract inherits `forge-std`'s `Test`, functions prefixed `test` are run, and **`vm` cheatcodes** manipulate the EVM (set the caller, warp time, expect reverts, fund accounts). `setUp()` runs before each test." },
        ...
        { type: "table", headers: ["Cheatcode", "Effect"], rows: [
          ...
          ["`vm.expectRevert(...)`", "assert the next call reverts (by string, selector, or bytes)"],
          ["`vm.expectEmit(...)`", "assert an event is emitted with expected topics/data"],
          ["`vm.mockCall(...)`", "stub an external call's return value"],
          ["`vm.load` / `vm.store`", "read/write raw storage slots directly"],
          ["`assertEq` / `assertGt` / `assertTrue`", "forge-std assertions (with nice diffs)"]
        ] },
        ...
        { type: "callout", variant: "tip", text: "`forge test -vvv` prints call traces on failing tests (`-vvvv` for all); `forge coverage` reports line/branch coverage; `forge fmt` formats; `forge test --match-test test_Deposit` runs one test. Configure fuzz `runs` per profile in `foundry.toml` (light locally, heavy in CI)." }
      ]
    },
    {
      id: "cast-anvil",
    ...
    },
    {
      ...
      body: [
        ...
        { type: "code", lang: "bash", code: "# simulate locally (no broadcast) — always do this first\nforge script script/Deploy.s.sol\n\n# broadcast to a network AND verify on Etherscan in one shot\nforge script script/Deploy.s.sol \\\n  --rpc-url $RPC_URL \\\n  --broadcast \\\n  --verify --etherscan-api-key $ETHERSCAN_API_KEY\n\n# quick one-off deploy without a script\nforge create src/MyToken.sol:MyToken \\\n  --constructor-args 1000000000000000000000000 \\\n  --private-key $PK --rpc-url $RPC --verify\n\n# verify an already-deployed contract\nforge verify-contract 0xAddress src/MyToken.sol:MyToken \\\n  --chain mainnet --etherscan-api-key $ETHERSCAN_API_KEY" },
        { type: "list", items: [
          "**`forge script` without `--broadcast`** just simulates (dry run) — inspect the trace and gas before committing.",
          "**Verification** publishes your source to Etherscan so users can read/interact with the contract; `--verify` does it inline, or run `forge verify-contract` after the fact.",
          "**Secrets:** load keys from env or a keystore (`cast wallet import`), never commit them. Foundry supports hardware wallets and `--account` keystores for production.",
          "**Broadcast artifacts** land in `broadcast/` — a record of every deploy with addresses and tx hashes."
        ] },
  ...
  cheatsheet: [
    ...
    { label: "Deploy one-off", code: "forge create src/T.sol:T --private-key $PK --rpc-url $RPC" }
  ]
});
